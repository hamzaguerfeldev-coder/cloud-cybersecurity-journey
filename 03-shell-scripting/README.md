...
| 1 | Hello script | First script: print a message with echo | [hello.sh](scripts/hello.sh "scripts/hello.sh") |   
| 2 | Show the date | Use command substitution $(date) inside a message | [date_show.sh](scripts/date_show.sh "scripts/date_show.sh") |   
| 3 | Addition | Sum two numbers passed as **arguments** ($1, $2) | [Addition.sh](scripts/Addition.sh "scripts/Addition.sh") |   
| 4 | Compare | Compare two arguments: greater / smaller / equal | [compare.sh](scripts/compare.sh "scripts/compare.sh") |   
| 5 | Even or odd | Check parity of an argument with arithmetic $((%)) | [pair.sh](scripts/pair.sh "scripts/pair.sh") |   
| 6 | Multiplication table | for loop over a number given by the user (read) | [multiple.sh](scripts/multiple.sh "scripts/multiple.sh") |   
| 7 | Counter | Print all numbers from 0 to N with a for loop | [compte.sh](scripts/compte.sh "scripts/compte.sh") |   
| 8 | Count files & dirs | find + wc -l to count files and directories in the current path | [how_much_file.sh](scripts/how_much_file.sh "scripts/how_much_file.sh") |   
| 9 | Does it exist? | Test a path: file (-f), directory (-d), or nothing | [exist_file.sh](scripts/exist_file.sh "scripts/exist_file.sh") |   
| 10 | Permissions | Check if a file is readable / writable / executable (-r, -w, -x) | [permission.sh](scripts/permission.sh "scripts/permission.sh") |   
| 11 | Word search | Verify a word exists inside a file with grep -q | [search_word.sh](scripts/search_word.sh "scripts/search_word.sh") |   
| 12 | File analysis | Count lines, words, chars of a file with wc | [analyse.sh](scripts/analyse.sh "scripts/analyse.sh") |   
| 13 | Dated directory | Build a dynamic name with $(date +%Y-%m-%d) and create it | [dynamic_directory.sh](scripts/dynamic_directory.sh "scripts/dynamic_directory.sh") |   
